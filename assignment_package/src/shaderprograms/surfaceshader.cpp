...
SurfaceShader::SurfaceShader(OpenGLContext *context)
    : ShaderProgram(context),
      attrPos(-1), attrNor(-1), attrUV(-1), unifModel(-1),
      unifModelInvTr(-1), unifView(-1), unifProj(-1), unifcamPos(-1),
      unifLook(-1), unifDimension(-1)
{}
...
void SurfaceShader::setCamPos(const glm::vec3 &eye, const glm::vec3 &look) {
    useMe();
...
void SurfaceShader::setDimension(const glm::ivec2 &dim) {
    useMe();

    if(unifDimension != -1) {
        context->glUniform2iv(unifDimension,
                              1,
                              &dim[0]);
    }
}
