...
void MyGL::resizeGL(int w, int h)
{
    ...
    for(std::shared_ptr<PostProcessShader> p : m_postprocessShaders)
    {
        ...
        mp_progSurfaceCurrent->setDimension(glm::ivec2(w, h));
    }
...
void MyGL::render3DScene()
{
    ...
    mp_progSurfaceCurrent->setCamPos(m_camera.eye, m_camera.look);
...
void MyGL::createShaders()
{
    ...
    std::shared_ptr<PostProcessShader> extra = std::make_shared<PostProcessShader>(this);
    extra->create(":/glsl/post/passthrough.vert.glsl", ":/glsl/post/extra.frag.glsl");
    m_postprocessShaders.push_back(extra);
